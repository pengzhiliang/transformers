...
def main():
    ...
    model.eval()
    print('Old noise_scheduler:', model.model.noise_scheduler.config)

    # breakpoint()
    # cfg_scale = 1.3
    # model.model.noise_scheduler = DPMSolverMultistepScheduler(
    #     num_train_timesteps=model.model.noise_scheduler.config.num_train_timesteps,
    #     beta_schedule='squaredcos_cap_v2',
    #     prediction_type=model.model.noise_scheduler.config.prediction_type,
    # )
    # model.set_ddpm_inference_steps(num_steps=5)

    cfg_scale = 2
    model.model.noise_scheduler = DDIMScheduler(
        num_train_timesteps=model.model.noise_scheduler.config.num_train_timesteps,
        beta_schedule='squaredcos_cap_v2',
        prediction_type=model.model.noise_scheduler.config.prediction_type,
        clip_sample=False,
    ...
    print('New noise_scheduler:', model.model.noise_scheduler.config)
    ...
    for step in [50]:
        ...
        for cfg_scale in np.arange(1.5, 3.0, 0.5):
            ...
            for i, save_name in enumerate(save_names):
                ...
                output_path = f"/mnt/conversationhub/zhiliang/exp/podcast_eval/{model_name}/batch_refresh_negative_ddim_cfg_{cfg_scale}_step_{step}/{save_name}.wav"
                # output_path = f"./vibepod_outputs/ddim_cfg_{cfg_scale}_{save_name}.wav"
